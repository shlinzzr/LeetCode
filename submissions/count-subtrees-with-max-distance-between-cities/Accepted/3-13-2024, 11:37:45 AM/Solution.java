...
class Solution {
    ...
    public int[] countSubgraphsForEachDiameter(int n, int[][] edges) {
        ...
        for (int state=1; state<(1<<n); state++) {
            ...
            System.out.println("state="+Integer.toBinaryString(state) + " v1="+v1 + " v2="+v2);
            //在 state 下 (bitmask 列舉出 connected nodes 各種排列組合, 去掉不合法的 p.s 用count驗證)
            // 在這些Node中的 maximum distance 表示為此組合的subtree
