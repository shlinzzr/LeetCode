    ...
    public List<List<Integer>> palindromePairs(String[] words) {
        ...
        for(int i=0; i<len; i++){
            ...
            for(int k : set){ // from short to long
                ...
                if(map.containsKey(reverse.substring(0, k)) && isPalindrome(reverse, k, wLen-1)){
                    res.add(Arrays.asList(map.get(reverse.substring(0, k)), i));
                }
                ...
                // then we find "sss"
                if(isPalindrome(reverse, 0, wLen-k-1) && map.containsKey(reverse.substring(wLen-k, wLen))){
                    res.add(Arrays.asList(i, map.get(reverse.substring(wLen-k, wLen))));
                }
            }            
        }
