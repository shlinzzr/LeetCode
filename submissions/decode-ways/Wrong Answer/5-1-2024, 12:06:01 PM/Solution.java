...
class Solution {
    ...
    */
           ...
           if(idx+1 < s.length()){
            ...
            res+= helper(s, idx+1, memo);
            if(idx+2<=s.length()) res+= helper(s, idx+2, memo);
       }
    // you have not hsnfled any condition apart from 2 1 0 number can be 4 7 3 also
    // hi there ? able to hear?
    // there are 2 condition one ch can be 1 of 2 of 0 or ch can be any number apart from 0, 1, 2
    //   when ch is 1 for that we can slelect 
