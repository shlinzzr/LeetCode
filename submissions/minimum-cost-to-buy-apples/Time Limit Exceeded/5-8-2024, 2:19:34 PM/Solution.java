...
class Solution {
    public long[] minCost(int n, int[][] roads, int[] appleCost, int k) {
        ...
        for(int start=1; start<=n; start++){
            ...
            seen[start]=true;
            long rt = dfs(start, 0L, adjs, appleCost, k, seen);
            res[start-1] = rt;
        }
    ...
    private long dfs(int curr, long acc,  Map<Integer, Map<Integer, Integer>> adjs , int[] appleCost, int k, boolean[] seen){
        ...
        for(int neb : nebs.keySet()){
            ...
            long rt = dfs(neb, acc+roadCost, adjs, appleCost, k, seen);
